...
app.post('/api/registerNewUser', async (req, res) => {
    ...
    try { 
        if(!validateUserPassword(passWord)){
            ...
            users[userName]={username:userName, id :user_id++, password: passWord, cart: [] }
...
function validateUserPassword(password){
    return containsNumber(password) && isLongEnough(password) && isNotTooLong(password) &&containsCapitalLetter(password) && notCaontainsEvalReg(password);
}
...
app.get('/api/removeProductFromCart', async (req, res) => {
    ...
    let productId = req.query.productId;
    let user = get_user_by_id(userId)
    let newCart = user.cart.filter(element=> element.id!=parseInt(productId))
...
app.post('/api/addNewProduct', async (req, res) => {
    ...
    if (userName == "admin"){
        ...
        try {
            ...
            if (product === null) {
                let newProduct = {id:product_id++, name: name, price: price, description: description, image: image };
